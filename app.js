  ...
  for (let i = 0, len = squares.length; i < len; i++) 
    ...
    (function(index) {
      ...
      squares[i].onclick = function() {
        //if the square below your current is taken then you can go on top of it
        if(squares[index + 7].classList.contains('taken')) {
          ...
          //if the square below is not taken, you can't go on top of it
   ...
   for(let y = 0; y < winningArrays.length; y++) {
     ...
     if(square1.classList.contains('player-one') &&
        ...
        square4.classList.contains('player-one')) {
          //if they do then player-one wins
    ...
    square4.classList.contains('player-two')) {
      //if they do then player-two wins
      result.innerHTML = 'Player Two Wins!'
    }
   }
}

//add an event listener to every square that will trigger the checkBoard function on click
